...
class StatusCheckByNinoRequestFormSpec extends PlaySpec with OptionValues {

  val form: Form[StatusCheckByNinoFormModel] = new StatusCheckByNinoFormProvider()()
  ...
  "form" must {
    "bind" when {
      ...
      //gen year?
      "year is 2 digit, default add 19XX" in pending
    }
    ...
    "fail to bind" when {
      ...
      //todo generators for string inputs
      "givenName is empty" in pending
      "givenName is too short" in pending
      "givenName contains invalid chars" in pending

      "familyName is empty" in pending
      "familyName is too short" in pending
      "familyName contains invalid chars" in pending

      "nino is empty" in pending
      "nino is invalid" in pending
    }
  }
