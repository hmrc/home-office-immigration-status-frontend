...
class SessionCacheServiceImpl @Inject()(
  ...
  def get(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Option[StatusCheckByNinoFormModel]] =
    withSessionId { sessionId =>
      sessionCacheRepository
        .findById(sessionId)
        .map(
          _.flatMap(formQueryModel => encrypter.decryptFormModel(formQueryModel.data, sessionId, secretKey))
        )
    }
