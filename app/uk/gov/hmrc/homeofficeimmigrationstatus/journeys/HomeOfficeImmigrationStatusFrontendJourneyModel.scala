...
package uk.gov.hmrc.homeofficeimmigrationstatus.journeys
...
object HomeOfficeImmigrationStatusFrontendJourneyModel extends JourneyModel {
  ...
  object Transitions {
    ...
    def showStatusCheckByNino(user: String): HomeOfficeImmigrationStatusFrontendJourneyModel.Transition = Transition {
    ...
    def submitStatusCheckByNino(
      ...
      query: StatusCheckByNinoRequest): HomeOfficeImmigrationStatusFrontendJourneyModel.Transition =
      Transition {
          ...
          checkStatusByNino(extendedQuery).flatMap {
            ...
            case StatusCheckResponse(correlationId, _, Some(result)) =>
              result.statuses match {
                case Nil => goto(StatusNotAvailable(correlationId, query))
                case _   => goto(StatusFound(correlationId, query, result))
              }
