...
trait TestJourneyService[RequestContext] extends PersistentJourneyService[RequestContext] {
  ...
  def getBreadcrumbs(
    implicit requestContext: RequestContext,
    timeout: Duration,
    ec: ExecutionContext): List[model.State] = get.get._2

  protected def fetch(
    implicit requestContext: RequestContext,
    ec: ExecutionContext): Future[Option[(model.State, List[model.State])]]

  protected def save(s: (model.State, List[model.State]))(
    implicit requestContext: RequestContext,
